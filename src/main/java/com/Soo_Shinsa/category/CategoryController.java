...
public class CategoryController {
    ...
    public ResponseEntity<CommonResponse<Page<FindCategoryResponseDto>>> findByBrandId(@PathVariable Long brandId,
                                                                       @RequestParam(defaultValue = "0") int page,
                                                                       @RequestParam(defaultValue = "10") int size) {
        Page<FindCategoryResponseDto> findCategoryByBrandId = categoryService.findByBrandId(brandId, page, size);
        CommonResponse<Page<FindCategoryResponseDto>> response = new CommonResponse<>(ResponseMessage.CATEGORY_SELECT_SUCCESS,findCategoryByBrandId);
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }
